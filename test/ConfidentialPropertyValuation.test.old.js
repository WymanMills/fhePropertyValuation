...
describe("ConfidentialPropertyValuation - Comprehensive Test Suite", function () {
  ...
  describe("🔓 Valuation Access and Retrieval", function () {
    beforeEach(async function () {
      const fixture = await loadFixture(deployPropertyValuationFixture);
      this.propertyValuation = fixture.propertyValuation;
      this.owner = fixture.owner;
    ...
    it("should allow property owner to get encrypted valuation", async function () {
      await expect(
        this.propertyValuation.connect(this.propertyOwner1).getEncryptedValuation(1)
      ).to.not.be.reverted;
    });

    it("should allow valuator to get encrypted valuation", async function () {
      await expect(
        this.propertyValuation.connect(this.valuator1).getEncryptedValuation(1)
      ).to.not.be.reverted;
    });

    it("should reject unauthorized user getting encrypted valuation", async function () {
      await expect(
        this.propertyValuation.connect(this.propertyOwner2).getEncryptedValuation(1)
      ).to.be.revertedWith("Not authorized to access");
    });

    it("should reject getting non-existent valuation", async function () {
      await expect(
        this.propertyValuation.connect(this.propertyOwner1).getEncryptedValuation(999)
      ).to.be.revertedWith("Valuation not found");
    });

    it("should allow property owner to mark valuation as revealed", async function () {
      await expect(
        this.propertyValuation.connect(this.propertyOwner1).markValuationRevealed(1, 500000, 90)
      ).to.emit(this.propertyValuation, "ValuationRevealed")
        .withArgs(1, 500000, 90);
    });

    it("should reject unauthorized user marking valuation as revealed", async function () {
      await expect(
        this.propertyValuation.connect(this.propertyOwner2).markValuationRevealed(1, 500000, 90)
      ).to.be.revertedWith("Not authorized to reveal");
    });

    it("should reject double reveal", async function () {
      await this.propertyValuation.connect(this.propertyOwner1).markValuationRevealed(1, 500000, 90);
